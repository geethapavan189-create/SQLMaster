...
def get_all_quizzes():
    ...
    return [
        {
            "quiz": Quiz(title="SQL Fundamentals", description="Test your knowledge of basic SQL concepts — SELECT, WHERE, ORDER BY, and more.", difficulty="beginner", category="Fundamentals", time_limit=300, passing_score=60),
            "questions": [
                ...
                QuizQuestion(question="Which clause filters rows?", question_type="mcq", options=["ORDER BY", "GROUP BY", "WHERE", "HAVING"], correct_answer="WHERE", explanation="WHERE filters individual rows based on conditions.", points=10, order_index=4),
                ...
                QuizQuestion(question="Which is the correct order?", question_type="mcq", options=["SELECT FROM WHERE ORDER BY", "FROM SELECT WHERE ORDER BY", "SELECT WHERE FROM ORDER BY", "FROM WHERE SELECT ORDER BY"], correct_answer="SELECT FROM WHERE ORDER BY", explanation="The correct SQL syntax order is: SELECT columns FROM table WHERE condition ORDER BY column.", points=10, order_index=10),
            ]
        },
        {
            "quiz": Quiz(title="JOINs Mastery", description="Test your understanding of INNER JOIN, LEFT JOIN, and multi-table queries.", difficulty="intermediate", category="Joins", time_limit=420, passing_score=70),
            "questions": [
                ...
                QuizQuestion(question="What does LEFT JOIN return for unmatched rows?", question_type="mcq", options=["Nothing", "NULL values for right table columns", "An error", "Zero values"], correct_answer="NULL values for right table columns", explanation="LEFT JOIN keeps all left table rows. Unmatched right-side columns become NULL.", points=10, order_index=2),
                ...
                QuizQuestion(question="How do you find customers WITHOUT orders?", question_type="mcq", options=["INNER JOIN ... WHERE order IS NULL", "LEFT JOIN ... WHERE order.id IS NULL", "RIGHT JOIN ... WHERE customer IS NULL", "CROSS JOIN"], correct_answer="LEFT JOIN ... WHERE order.id IS NULL", explanation="LEFT JOIN keeps all customers, then WHERE o.id IS NULL finds those with no matching order.", points=10, order_index=4),
                QuizQuestion(question="What is a table alias?", question_type="mcq", options=["A backup table name", "A short name for a table in a query", "A view", "A foreign key"], correct_answer="A short name for a table in a query", explanation="Aliases (like 'e' for employees) make queries shorter and more readable.", points=10, order_index=5),
            ]
        },
        {
            "quiz": Quiz(title="Aggregation & Grouping", description="Test GROUP BY, HAVING, and aggregate functions.", difficulty="intermediate", category="Aggregation", time_limit=360, passing_score=70),
            "questions": [
                QuizQuestion(question="Which function counts rows?", question_type="mcq", options=["SUM()", "COUNT()", "TOTAL()", "NUM()"], correct_answer="COUNT()", explanation="COUNT() counts the number of rows. COUNT(*) counts all, COUNT(col) skips NULLs.", points=10, order_index=1),
                ...
                QuizQuestion(question="What's the rule for GROUP BY?", question_type="mcq", options=["All columns must be grouped", "Only aggregated columns need GROUP BY", "Non-aggregated SELECT columns must be in GROUP BY", "GROUP BY is optional"], correct_answer="Non-aggregated SELECT columns must be in GROUP BY", explanation="Every column in SELECT that isn't inside an aggregate function must appear in GROUP BY.", points=10, order_index=3),
