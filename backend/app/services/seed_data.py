...
def get_all_quizzes():
    """Return quiz data with 10 questions per chapter."""
    return [
        {
            "quiz": Quiz(title="SQL Fundamentals", description="Test your knowledge of basic SQL concepts — SELECT, WHERE, ORDER BY, and more.", difficulty="beginner", category="Fundamentals", time_limit=600, passing_score=75),
            "questions": [
        ...
        {
            "quiz": Quiz(title="JOINs & Relationships", description="Test your understanding of INNER JOIN, LEFT JOIN, and combining data from multiple tables.", difficulty="intermediate", category="Joins", time_limit=600, passing_score=75),
            "questions": [
                ...
                QuizQuestion(question="INNER JOIN and JOIN are:", question_type="mcq", options=["Different operations", "The same thing", "JOIN is faster", "INNER JOIN returns more rows"], correct_answer="The same thing", explanation="JOIN without a prefix defaults to INNER JOIN. They are identical.", points=10, order_index=6),
                QuizQuestion(question="What does FULL OUTER JOIN return?", question_type="mcq", options=["Only matching rows", "All rows from left table only", "All rows from both tables, with NULLs for non-matches", "Cartesian product"], correct_answer="All rows from both tables, with NULLs for non-matches", explanation="FULL OUTER JOIN returns all rows from both tables, filling NULLs where there's no match.", points=10, order_index=7),
                QuizQuestion(question="What is a self-join?", question_type="mcq", options=["Joining a table with itself", "Joining without ON clause", "Joining 3+ tables", "An automatic join"], correct_answer="Joining a table with itself", explanation="A self-join joins a table to itself, useful for hierarchical data like employee-manager relationships.", points=10, order_index=8),
                QuizQuestion(question="What does CROSS JOIN produce?", question_type="mcq", options=["Only matching rows", "Cartesian product (every combination)", "Union of both tables", "Intersection of tables"], correct_answer="Cartesian product (every combination)", explanation="CROSS JOIN combines every row from table A with every row from table B.", points=10, order_index=9),
                QuizQuestion(question="Which JOIN would you use to get ALL employees including those without a department?", question_type="mcq", options=["INNER JOIN", "LEFT JOIN (employees LEFT JOIN departments)", "RIGHT JOIN", "CROSS JOIN"], correct_answer="LEFT JOIN (employees LEFT JOIN departments)", explanation="LEFT JOIN keeps all rows from the left table (employees) even if they have no matching department.", points=10, order_index=10),
            ]
        },
        {
            "quiz": Quiz(title="Aggregation & Grouping", description="Test GROUP BY, HAVING, COUNT, SUM, AVG, and aggregate functions.", difficulty="intermediate", category="Aggregation", time_limit=600, passing_score=75),
            "questions": [
        ...
        {
            "quiz": Quiz(title="Advanced SQL", description="Window functions, CTEs, subqueries, and advanced concepts.", difficulty="advanced", category="Advanced", time_limit=720, passing_score=75),
            "questions": [
